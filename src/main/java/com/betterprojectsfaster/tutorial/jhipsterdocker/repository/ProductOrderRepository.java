...
public interface ProductOrderRepository extends JpaRepository<ProductOrder, Long> {
    @Query("select productOrder from ProductOrder productOrder where productOrder.buyer.login = ?#{authentication.name}")
    ...
    @Query(
        value = "select productOrder from ProductOrder productOrder left join fetch productOrder.product left join fetch productOrder.buyer left join fetch productOrder.overallOrder",
    ...
    @Query(
        "select productOrder from ProductOrder productOrder left join fetch productOrder.product left join fetch productOrder.buyer left join fetch productOrder.overallOrder"
    )
