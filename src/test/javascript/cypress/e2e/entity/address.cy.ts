...
describe('Address e2e test', () => {
  ...
  // const addressSample = {"addressLine1":"mammoth"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"JDY@FZt\\.jN","firstName":"Rusty","lastName":"Hamill","email":"Demarco.Stark@gmail.com","imageUrl":"cache pish inhabitant","langKey":"pant"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  describe('new Address page', () => {
    ...
    it.skip('should create an instance of Address', () => {
      cy.get(`[data-cy="addressLine1"]`).type('optimistically');
      cy.get(`[data-cy="addressLine1"]`).should('have.value', 'optimistically');

      cy.get(`[data-cy="addressLine2"]`).type('insubstantial terribly');
      cy.get(`[data-cy="addressLine2"]`).should('have.value', 'insubstantial terribly');

      cy.get(`[data-cy="city"]`).type('Eau Claire');
      cy.get(`[data-cy="city"]`).should('have.value', 'Eau Claire');

      cy.get(`[data-cy="postalCode"]`).type('thin ');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'thin ');
